...
void W25Qx_Read_ID(uint8_t *ID)
{
	...
	/* Reception of the data */
	HAL_SPI_Receive(&hspi1,ID, 2, W25QXXXX_TIMEOUT_VALUE);
	W25Qx_Disable();
...
uint8_t W25Qx_WriteDMA(uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
	uint8_t cmd[4];
	uint32_t end_addr, current_size, current_addr;
	uint32_t tickstart = get_tick();
  ...
  do
  {
    ...
    /* Transmission of the data */
	 if (spi1_write_data(pData,current_size) != HAL_OK)
    //if (HAL_SPI_Transmit(&hspi1, pData,current_size, W25QXXXX_TIMEOUT_VALUE) != HAL_OK)
    {
      return W25Qx_ERROR;
    }
