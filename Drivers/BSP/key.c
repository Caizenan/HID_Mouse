...
//#endif
keyset_t keyset;
void StartKeyCtrlTask(void const * argument); 
...
{
	  //因为按键开启为高 所以相反
    if (HAL_GPIO_ReadPin(Key_9_GPIO_Port, Key_9_Pin) == 1)
    {
...
{
	  //因为按键开启为高 所以相反
    if (HAL_GPIO_ReadPin(Key_4_GPIO_Port, Key_4_Pin) == 1)
    {
        return KEY_READ_DOWN;
    }
    else
...
unsigned char key2_read(void)
{
	  //因为按键开启为高 所以相反
    if (HAL_GPIO_ReadPin(Key_11_GPIO_Port, Key_11_Pin) == 1)
    {
        return KEY_READ_DOWN;
    }
    else
...
unsigned char key8_read(void)
{
	  //因为按键开启为高 所以相反
    if (HAL_GPIO_ReadPin(Key_5_GPIO_Port, Key_5_Pin) == 1)
    {
        return KEY_READ_DOWN;
    }
    else
...
unsigned char key9_read(void)
{
	  //因为按键开启为高 所以相反
    if (HAL_GPIO_ReadPin(Key_3_GPIO_Port, Key_3_Pin) == 1)
    {
        return KEY_READ_DOWN;
    }
    else
...
data : 2017-03-02
*/
void key_write_value(unsigned short key_val)
{   
